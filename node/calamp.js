...
var Server = function(){
    ...
    this.process_dgram = function(datagram,guest){
        ...
        function getDeviceId(imei,packet, callback)
        {
            ...
            s.mysql_link.query('SELECT id,client_id,travel_id,name,geofences,boxs_id,stop,stop_time,bad_engine,engine  FROM devices WHERE imei='+imei, function(err, rows ,fields)
            {
                        ...
                        else
                        {
                        	...
                        	s.mysql_link.query('SELECT * FROM packets WHERE devices_id ='+packet.device_id +' ORDER BY id DESC LIMIT 1', function(err, rows, fields){
		                        if(isEmpty(rows)){ 
                                    ...
                                    console.log('PREVIO ->' + packet.previous_heading)
